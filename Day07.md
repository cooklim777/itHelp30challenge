...
同一個模組也可以使用不同的觸發器，以 google sheet 為例，可以看到 Google sheet 模組內有非常多的方法可以取用，包括 sheet、row、cell 相關，每一個都是可以當開頭的（都可以時間觸發、一次觸發等），可以當你點擊觸發時就要新增一個 sheet 也是可以 但就是比較奇怪。通常會當觸發的就是我圈起來的兩個，一個是讀取資料，而一個是觀察資料觸發時有新資料就會得到。
